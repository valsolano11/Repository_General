...
const ConsumoFicha = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await api.get("/productos/categoria/:3", {
            headers: {
            ...
            unidadNombre: produc.UnidadMedida ? produc.UnidadMedida.sigla : "Desconocido",
            }));
        ...
        {
            name: "nombre",
        label: "Nombre del Producto", // Cambia la etiqueta si es necesario
        options: {
            customHeadRender: (columnMeta) => (
            <th className="text-center bg-white text-black uppercase text-xs font-bold">{columnMeta.label}</th>
            ),
        ...
        {
        name: "unidadNombre", // Cambiado a "unidadNombre"
